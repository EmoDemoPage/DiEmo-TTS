...
def dino_load_ckpt(cur_model, ckpt_base_dir, model_name='model', force=True, strict=False):
    if os.path.isfile(ckpt_base_dir):
        base_dir = os.path.dirname(ckpt_base_dir)
        ckpt_path = ckpt_base_dir
        checkpoint = torch.load(ckpt_base_dir, map_location='cpu')
    ...
    if checkpoint is not None:
        ...
        for k, v in state_dict.items():
            if k.startswith("module.backbone"):
                new_key = k.replace("module.backbone", "student_encoder", 1)
                new_state_dict[new_key] = v
            else:
                new_state_dict[k] = v
        state_dict = new_state_dict  # 변환된 state_dict로 업데이트
