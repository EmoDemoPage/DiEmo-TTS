...
class BaseBinarizer:
    ...
    def process_item(cls, item, binarization_args):
        ...
        try:
            ...
            if binarization_args["with_align"]:
                tg_fn = (
                    f"{hparams['processed_data_dir']}/mfa_outputs/{item_name}.TextGrid"
                )
                item["tg_fn"] = tg_fn
                ...
                if binarization_args["trim_eos_bos"]:
                    ...
                    item["wav"] = wav[
                        n_bos_frames * hparams["hop_size"] : len(wav)
                        - n_eos_frames * hparams["hop_size"]
                    ]
    ...
    def process_align(tg_fn, item):
        ...
        # print("tokenshape", len(ph_tokenm), "mel2ph", np.array(mel2ph).max() - 1)
        print(len(ph_token) - (np.array(mel2ph).max() - 1))
