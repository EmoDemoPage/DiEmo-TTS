...
class MelStyleEncoder(nn.Module):
    ...
    def forward(self, x, x_lengths=None):   # [B, L, D] 
        mask = None
        slf_attn_mask = None
        if x_lengths is not None:    
            mask = self.get_mask_from_lengths(x_lengths)
            max_len = x.shape[1]
        ...
        w = self.temporal_avg_pool(x, mask=None)

        return w
    
    
class DINOHead(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn=False, norm_last_layer=True, nlayers=3, hidden_dim=1024, bottleneck_dim=64):
        super().__init__()
